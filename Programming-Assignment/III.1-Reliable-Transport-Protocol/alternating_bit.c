...
{
    if (!busy) {
        A_sndpck = make_pkt(message, send_seq);
        A_udt_send(A_sndpck);
        busy = true;
        A_starttimer();
    }
...
void A_input(revpkt)
  struct pkt revpkt;
{
    if (is_corrupt(revpkt) || !is_cur_seq_ack(revpkt, send_seq)) {
        // do nothing
    } else {
//        printf("A: success=%d\n", revpkt.acknum);
        busy = false;
  ...
  struct pkt rvcpkt;
{
    ...
    if (is_corrupt(rvcpkt) || !is_cur_seq_pkt(rvcpkt, excpeted_seq)) {
        int prev_seq = 1 - excpeted_seq;
        sndpkt = make_ack(prev_seq);
    } else {
        struct msg message = extract_msg(rvcpkt);
        B_delivery_msg(message);
        sndpkt = make_ack(excpeted_seq);
        excpeted_seq = 1 - excpeted_seq;
    }
    B_udt_send(sndpkt);
}
